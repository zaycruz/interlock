...
export function stagedPaths(repositoryPath: string): string[] {
  ...
  for (let index = 0; index < tokens.length;) {
    index = readStagedChange(tokens, index, paths);
  }
  return paths;
}

function readStagedChange(tokens: string[], index: number, paths: string[]): number {
  ...
  for (let offset = 0; offset < pathCount; offset += 1) {
    ...
    // A copy does not change its source. A rename changes both paths.
    if (!status.startsWith("C") || offset !== 0) paths.push(path);
  }
  return index;
}
